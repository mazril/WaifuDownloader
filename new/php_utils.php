...
function get_priority_queue_db() {
    ...
    $items = [];
    try {
        $stmt = $pdo->query("SELECT item_type, item_data, priority FROM priority_queue ORDER BY priority ASC, added_timestamp ASC");
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $decoded_data = json_decode($row['item_data'], true);
            $items[] = ["type" => $row['item_type'], "payload" => $decoded_data, "priority" => $row['priority']];
        }
    } catch (PDOException $e) {
...
function update_priority_queue_db($queue_data) {
    ...
    try {
        ...
        foreach ($queue_data as $index => $item) {
            if (!isset($item['type']) || !array_key_exists('payload', $item)) {
                error_log("Nieprawidłowy format elementu kolejki podczas update_priority_queue_db: " . print_r($item, true));
                continue; 
            }
            $item_data_json_q_update = json_encode($item['payload']);
            if ($item_data_json_q_update === false) {
                error_log("Błąd kodowania JSON w update_priority_queue_db dla: " . print_r($item['payload'], true));
                continue; 
            }
            $priority_to_save = isset($item['priority']) ? (int)$item['priority'] : ($index * 10);
            
            $stmt->execute([
                ':item_type' => $item['type'],
                ':item_data_q_update' => $item_data_json_q_update,
                ':priority' => $priority_to_save 
            ]);
        }
        $pdo->commit();
        return true;
...
/**
 * Odczytuje i dekoduje plik konfiguracyjny Pythona (config.json).
 * Opis: Nowa funkcja do centralnego odczytu konfiguracji Pythona w PHP.
 * Wpływ na inne funkcje: Używana przez ai_worker.php i api.php do pobierania
 * ustawień, które mogą być modyfikowane przez użytkownika.
 * @return array|null Zdekodowana tablica konfiguracyjna lub null w przypadku błędu.
 */
function get_python_config() {
    ...
    if (!file_exists($config_path) || !is_readable($config_path)) {
        error_log("Błąd krytyczny: Brak pliku config.json lub brak uprawnień do odczytu.");
        return null;
    }
    
    $config_content = file_get_contents($config_path);
    $decoded_config = json_decode($config_content, true);
    
    if (json_last_error() !== JSON_ERROR_NONE) {
        error_log("Błąd dekodowania JSON w pliku config.json: " . json_last_error_msg());
        return null;
    }
