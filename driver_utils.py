...
def create_driver_with_retry(shutdown_flag_func=None):
    for attempt in range(1, constants.MAX_DRIVER_STARTUP_ATTEMPTS + 1):
        if shutdown_flag_func and shutdown_flag_func():
            logger.info("create_driver_with_retry: Wykryto żądanie zamknięcia. Przerywam tworzenie drivera.")
            raise constants.RestartRequiredError("Tworzenie drivera przerwane przez żądanie zamknięcia.", no_vpn=True)
        ...
        join_timeout = constants.DRIVER_STARTUP_TIMEOUT
        start_join_time = time.monotonic()
        while creation_thread.is_alive():
            if shutdown_flag_func and shutdown_flag_func():
                logger.warning("create_driver_with_retry: Żądanie zamknięcia podczas oczekiwania na wątek tworzenia drivera.")
                raise constants.RestartRequiredError("Tworzenie drivera przerwane (wątek) przez żądanie zamknięcia.", no_vpn=True)
            
            remaining_timeout = join_timeout - (time.monotonic() - start_join_time)
            if remaining_timeout <= 0:
                break 
            creation_thread.join(timeout=min(1.0, remaining_timeout))
        ...
        if creation_thread.is_alive(): 
            ...
            try:
                ...
                if isinstance(result, Exception):
                    ...
                    if attempt >= constants.MAX_DRIVER_STARTUP_ATTEMPTS:
                        raise constants.RestartRequiredError("Nie udało się utworzyć instancji drivera (wątek zakończony bez rezultatu).")
                else: 
                ...
                if attempt >= constants.MAX_DRIVER_STARTUP_ATTEMPTS:
                    raise constants.RestartRequiredError("Nieznany błąd tworzenia drivera (pusta kolejka po zakończeniu wątku).")
...
def is_url_loaded(driver, target_url):
    """Sprawdza, czy aktualnie załadowany URL w przeglądarce pasuje do docelowego."""
    if not driver: return False
    try:
        current_url = driver.current_url
        return current_url.split('#')[0].rstrip('/') == target_url.split('#')[0].rstrip('/')
    except WebDriverException:
        logger.warning("is_url_loaded: WebDriverException podczas odczytu current_url.")
        return False
    except Exception as e:
        logger.warning(f"is_url_loaded: Nieoczekiwany błąd: {e}")
        return False
...
def safe_driver_get(driver, url, shutdown_flag_func=None):
    ...
    try:
        ...
        while time.monotonic() - pause_start_time < target_pause:
            if shutdown_flag_func and shutdown_flag_func():
                logger.info(f"safe_driver_get: Wykryto żądanie zamknięcia podczas pauzy po załadowaniu {url}.")
                raise constants.RestartRequiredError("Pauza po ładowaniu strony przerwana przez żądanie zamknięcia.", no_vpn=True)
            time.sleep(0.1) 

        logger.debug(f"Pauza po driver.get zakończona ({target_pause:.2f}s).")
...
def _is_element_in_viewport(driver, element, shutdown_flag_func=None):
    if shutdown_flag_func and shutdown_flag_func():
        logger.debug("_is_element_in_viewport: Wykryto żądanie zamknięcia, pomijam skrypt JS.")
        return False 
    try:
        ...
        logger.warning(f"Błąd podczas wykonywania skryptu JS (_is_element_in_viewport): {e}")
        return False
...
def scroll_until_timeout(driver, selector, expected_count=None, allow_up_scroll=True,
    ...
    while True:
        ...
        if refresh_count < max_r:
            ...
            for _ in range(up_r): 
                if shutdown_flag_func and shutdown_flag_func(): break
                driver.execute_script(f"window.scrollBy(0, -{jump});"); time.sleep(0.3)
            if shutdown_flag_func and shutdown_flag_func(): break
