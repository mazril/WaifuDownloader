...
def main_menu():
    ...
    if choice == '1':
        ...
        models_list = db_manager.execute_query("SELECT model_name FROM models ORDER BY model_name ASC", fetch_all=True)
        models_list_len = len(models_list) if models_list else 0
        ...
        if start_idx >= models_list_len and models_list_len > 0:
            ...
            if input("   Lista przetworzona. Zacząć od początku? (t/N): ").lower() == 't':
                ...
                logger.info("Nie rozpoczynam od początku. Powrót."); return None, None
        elif models_list_len == 0: 
            logger.warning("Lista modelek jest pusta."); return None, None
        params = {"start_model_index": start_idx, "check_mode": "all_or_incomplete"}; return "process_models", params
    elif choice == '2':
        logger.info("Przetwarzanie listy od początku.")
        current_state = db_manager.get_app_state('script_state') or {}
        current_state["last_model_index_processed"] = -1
        db_manager.update_app_state('script_state', current_state)
...
def main_loop():
    global shutdown_requested
    global collected_gallery_image_links # Udostępnienie globalnego słownika
        ...
        while not shutdown_requested:
            try:
                ...
                if active_op_name and not shutdown_requested:
                    ...
                    try:
                        if active_op_name == "process_models": 
                            ...
                            processing.handle_fill_incomplete(
                                shutdown_flag_func=lambda: shutdown_requested,
                                collected_gallery_image_links_ref=collected_gallery_image_links # Dodano, jeśli HFI też ma przetwarzać
                            )
                    ...
                    if operation_completed_normally:
                        if not db_manager.get_priority_queue() and not shutdown_requested:
                            ...
                            current_state["current_operation"] = {"name": None, "params": {}}
                            db_manager.update_app_state('script_state', current_state)
                            active_op_name = None; force_menu_next_iteration = True
            ...
            if force_menu_next_iteration and not active_op_name and not db_manager.get_priority_queue(): time.sleep(0.1)
    finally:
